...
If the response time ever shows up in baseline traces, a partial
index `WHERE preactive IS NOT NULL OR active IS NOT NULL` plus a
covering composite index `(preactive, active, pk)` would cure it.
**Don't pre-optimize** — file as a discretionary follow-up.
...
- **`/admin/library/<pk>` retrieve**. The same `comic_count` /
  ...
  queryset).
- **`AdminLibrarianStatusActiveViewSet` + `…AllViewSet` consolidation**.
  They differ only in filter and ordering. One viewset with a
  query-param toggle would simplify the URL conf and the OpenAPI
  schema. Not a perf win.
- **`UserAuth` / `GroupAuth` always-create-on-update pattern**.
  `_apply_userauth` calls `get_or_create` on every update; if the
  row should always exist post-creation, swap to `update`. Cosmetic.
...
4. **Folder picker (finding 4).** `os.scandir` rewrite. Small but
   self-contained.
5. **Hygiene checks (findings 5, 7).** Verify cachalot covers
   `HasAPIKeyOrIsAdminUser`; fix the `_TASK_MAP` mutation race.

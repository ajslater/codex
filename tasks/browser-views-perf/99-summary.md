...
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------- | ---------------------------------------------------------- | ------ | ---- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **Reduce cover endpoint pipeline cost.** Add a `for_cover=True` short-circuit to `get_filtered_queryset` that skips annotation / order aggregates. Alternatively pre-compute signed cover URLs in the BrowserView response so the cover endpoint never runs the filter pipeline. | 05-auxiliary #1            | **Very high** (saves seconds per page load × every browse) | M-L    | M    | ✅ **Stage 3** (correlated-subquery `cover_pk` + per-pk endpoint) and **Stage 5a** (`cache_page` on cover routes). Flow D warm 802 → 0 queries. |
| 2   | **Batch metadata intersection hydrations.** Replace the 11 sequential `field.related_model.objects.filter(pk__in=pks)` calls in `metadata/query_intersections.py:126` with a single `prefetch_related` on the union result.                                                      | 04-metadata #1             | **High**                                                   | M      | M    | ✅ **Stage 4** — `FK_QUERY_OPTIMIZERS` carries `select_related` + `.only()` hints; empty-intersection short-circuit via `.none()`.              |
| 3   | **Consolidate metadata value-field annotations.** Collapse the 3 sequential `_intersection_annotate()` calls in `metadata/annotate.py:144-164` into one.                                                                                                                         | 04-metadata #2             | **High**                                                   | S-M    | L    | ✅ **Stage 4** — three passes folded into one batched `aggregate()` + `values_list()` with synthetic keys.                                      |
| 4   | **Remove triple COUNT in browser flow.** Use `paginator.count`/`len(object_list)` instead of extra `.count()` calls in `paginate.py:68,70`; benchmark grouped-COUNT alternatives for `browser.py:94`.                                                                            | 01-core-browser-flow #1,#3 | **High**                                                   | M      | M    | ✅ **Stage 2** (PR 2a) — shadowed `Paginator.count`, derived page row count from `start_index`/`end_index`.                                     |

### Tier 2 — redundant work on every request

| #   | Change                                                                                                                                                                                                                | Sub-plan                | Impact                                            | Effort | Risk | Status                                                                                                                   |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- | ------------------------------------------------- | ------ | ---- | ------------------------------------------------------------------------------------------------------------------------ |
| 5   | **Cache bookmark-Max aggregate on `self`.** Single computation in `annotate/order.py`, `annotate/bookmark.py`, `group_mtime.py`.                                                                                      | 02-annotations #4       | High                                              | S      | L    | ✅ **Stage 1** — memoized per `(model, agg_func, default)` on the view instance; three call sites share one Aggregate.   |
| 6   | **Gate `.distinct()` on m2m-join presence.** `filters/filter.py:65` — track `_uses_m2m_join` during Q construction; only apply `.distinct()` when needed.                                                             | 03-filters #1           | High                                              | S-M    | M    | ✅ **Stage 5b** — `comic_filter_uses_m2m` cached property gates `.distinct()` on Comic querysets.                        |
| 7   | **Memoize search parse result.** Add `functools.lru_cache(256)` to `search/parse.py::_preparse_search_query` keyed on `(text, is_admin, folder_view_flag)`; add `lru_cache` to `search/field/filter.get_field_query`. | 03-filters #3,#4        | Medium-High                                       | S      | L    | ✅ **Stage 1** — `lru_cache(256)` on `_preparse_search_query`, `lru_cache(512)` on `get_field_query`.                    |
| 8   | **Diff-save params.** `params.py:60` writes `SettingsBrowser` on every request even when unchanged — causes cachalot flush. Compare to stored and skip when identical.                                                | 01-core-browser-flow #9 | Medium (cachalot flush impact is the hidden cost) | S      | L    | ✅ **Stage 1** — short-circuit on the four `_save_browser_*` paths when nothing changed.                                 |
| 9   | **Replace unconditional `JsonGroupArray` `updated_ats` with scalar `Max`** for non-browser targets (`annotate/card.py:79-83`). For browser, compute Max once per page and pass via serializer context.                | 02-annotations #1       | High                                              | M      | M    | ✅ **Stage 5b** — skipped entirely outside `CARD_TARGETS = {"browser", "metadata"}`; no scalar Max needed (no consumer). |
| 10  | **Cache `libraries_exist`.** `browser.py:206` — cache signal-based or short-TTL.                                                                                                                                      | 01-core-browser-flow #2 | Medium                                            | S      | L    | ✅ **Stage 1** — 60s TTL in default cache + signal invalidation on `Library` save/delete.                                |

### Tier 3 — batch-shaped redundancies

| #   | Change                                                                                                                                                                                                                                             | Sub-plan                | Impact                  | Effort | Risk | Status                                                                                                                                                                                                                                                             |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- | ----------------------- | ------ | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 11  | **Batch metadata group through-model queries** into a UNION. `metadata/query_intersections.py:22-46`.                                                                                                                                              | 04-metadata #3          | Medium                  | M      | M    | ⏳ **Open.** Not pursued in Stage 5; metadata cold path already dropped 31 → 28 queries via Stage 4 batched annotation. Revisit if metadata cold cost re-emerges.                                                                                                  |
| 12  | **Batch metadata FK intersection queries** into a UNION and batch-extract with `.first()`. `metadata/query_intersections.py:71-79`, `copy_intersections.py:63-65`.                                                                                 | 04-metadata #4,#5       | Medium                  | M      | M    | ⏳ **Open.** Stage 4 added FK hydration hints (`select_related` + `.only()`) which absorbed the bulk of the per-FK overhead; the explicit UNION batching wasn't needed for the wins.                                                                               |
| 13  | **Batch choices endpoint.** Convert from 26 sequential GETs to one `POST /choices {"fields": [...]}`; cache per (user, library, filters). Frontend change required.                                                                                | 06-choices #1           | Medium-High per session | M      | L    | ✅ **Stage 5c** — pure-backend variant landed: single batched `EXISTS` annotate (m2m split into has_rel/has_null + lazy distinct probe). 34 → 11 cold queries on `choices_available`.                                                                              |
| 14  | **Gate annotations to consuming targets.** In `annotate/order.py:261-274`: `ids` (JsonGroupArray) → opds2 only; `filename` → folder or `order_key == "filename"`; `search_score` `group_by("id")` → only when story_arc filter is active on Comic. | 02-annotations #2,#3,#5 | Medium                  | M      | M    | ✅ **Stage 5b** — `search_score` `group_by("id")` tied to `comic_filter_uses_m2m`; other gates landed earlier.                                                                                                                                                     |
| 15  | **Batch mtime endpoint queries.** `mtime.py:37-40` loops `get_group_mtime` per group; fold into one aggregate.                                                                                                                                     | 01-core-browser-flow #6 | Medium                  | M      | M    | ⏳ **Open.** `get_max_groups_mtime` still loops. Endpoint isn't on a hot Flow A-H path — defer until profiling shows it as a bottleneck.                                                                                                                           |
| 16  | **Cross-request ACL cache.** `filters/filter.py:34` calls `get_acl_filter` 7+ times per request; the underlying visible-libraries set changes rarely. Cache per `(user_id, library_max_updated_at)`.                                               | 03-filters #2           | Medium                  | M      | L    | ❌ **Skipped.** Subsumed by per-request scalar caching on `GroupACLMixin` (already cached `_cached_visible_library_pks`, `_cached_max_idx`, `_cached_default_fits`). Reconsider only if profiling shows visible-library computation as a cross-request bottleneck. |
| 17  | **Cache page mtime.** `browser.py:141` — short-TTL cache per `(user, group, pks, params)` or denormalize.                                                                                                                                          | 01-core-browser-flow #4 | Medium                  | S-M    | L    | ✅ **Stage 2** (PR 2b) — 5 s TTL cache layer keyed on `(user, model, group, pks, page, filter_hash)`; polling MtimeView path unaffected.                                                                                                                           |

### Tier 4 — clean-ups / small wins

| #   | Change                                                                                                            | Sub-plan                 | Impact     | Effort | Risk | Status                                                                                                                                                                                                                                              |
| --- | ----------------------------------------------------------------------------------------------------------------- | ------------------------ | ---------- | ------ | ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 18  | **Collapse `BrowserChoicesAvailableView` existence checks** into one SQL with `CASE WHEN`.                        | 06-choices #2            | Low        | S      | L    | ✅ **Stage 5c** (absorbed) — the batched `EXISTS` annotate in #13 collapsed the existence-check loop in the same pass.                                                                                                                              |
| 19  | **Pre-filter `get_all_comic_field_filters` loop** to active keys only. `filters/field.py:53-61`.                  | 03-filters #6            | Low        | S      | L    | ✅ **Stage 1** — pre-filter the field loop to keys actually set in the request; saves ~20 no-op calls per browser response.                                                                                                                         |
| 20  | **Cache `BaseDatabaseOperations(None)` at module level.** `search/field/expression.py:106`.                       | 03-filters #7            | Tiny       | XS     | L    | ✅ **Stage 1** — `_DB_OPS = BaseDatabaseOperations(None)` stashed module-level.                                                                                                                                                                     |
| 21  | **Pull `bmua_is_max` off the row** (move to serializer context). `annotate/order.py:198-203`.                     | 02-annotations #7        | Low        | S      | L    | ✅ **Stage 1** — flag moved off the per-row `Value` annotation; serializer reads from `self.context["view"]`.                                                                                                                                       |
| 22  | **Expand breadcrumbs `select_related`** to cover full FK chain per group type. `breadcrumbs.py:65-66`.            | 01-core-browser-flow #5  | Low-Medium | XS     | L    | ⏳ **Open.** Not addressed in Stage 1-5; sub-100-ms win, can be picked up in a future cleanup pass.                                                                                                                                                 |
| 23  | **Only add `add_group_by` once.** `browser.py:88,117` — dedupe.                                                   | 01-core-browser-flow #7  | Low        | XS     | L    | ✅ **Stage 1** — single `add_group_by` in `_get_common_queryset`; second call dropped from `_get_group_queryset`.                                                                                                                                   |
| 24  | **Guard `group_instance` query** on empty pks. `breadcrumbs.py:86-102`.                                           | 01-core-browser-flow #10 | Low        | XS     | L    | ⏳ **Open.** Not addressed in Stage 1-5; pairs naturally with #22 if breadcrumbs gets a future pass.                                                                                                                                                |
| 25  | **Skip `sort_name` alias fan-out** beyond the current parent_group. `mixins.py:56-70`.                            | 02-annotations #8        | Low        | S      | L    | 🔄 **Stage 5d (re-investigated)** — `_get_order_groups` already does parent-aware reduction for the common drill-down case; remaining fan-out feeds ORDER BY tie-breaks at root/folder/story-arc. Trimming further would change ordering semantics. |
| 26  | **Add sparse index on `metadata_mtime`** or cast to boolean at annotation time. `annotate/card.py:55-59`, models. | 02-annotations #9        | Low        | S      | L    | ✅ **Stage 5d** — boolean cast variant landed (`ExpressionWrapper(Q(metadata_mtime__isnull=False), BooleanField())`) for both card and reader-books annotations.                                                                                    |
| 27  | **Audit `annotate_group_names` for opds1.** Gate if opds1 spec doesn't use names. `mixins.py:88-111`.             | 02-annotations #11       | Low        | XS     | L    | 🔄 **Stage 5d (re-investigated)** — already gated via `_GROUP_NAME_TARGETS`.                                                                                                                                                                        |
| 28  | **Remove dead code: `search/field/optimize.py`.** Unused per sub-plan analysis.                                   | 03-filters cross-cutting | Low        | XS     | L    | ✅ **Stage 1** — `search/field/optimize.py` deleted (`like_qs_to_regex_q` and friends were unused; only `optimize.py` left in the tree is the librarian-scribe one).                                                                                |
| 29  | **Move multi-PK sum into main metadata queryset.** `metadata/__init__.py:52-97`.                                  | 04-metadata #6           | Low        | S      | L    | ⏳ **Open.** Not addressed in Stage 1-5; metadata batched annotation in Stage 4 absorbed the bulk of the cost.                                                                                                                                      |
| 30  | **Request-scope bookmark notification deduplication.** `bookmark.py:20-70`.                                       | 05-auxiliary #4          | Low        | S      | L    | 🔄 **Stage 5d (re-investigated)** — `BookmarkUpdateMixin.update_bookmarks` already emits exactly one `_notify_library_changed` per call.                                                                                                            |
| 31  | **Incremental download streaming.** `download.py` — use ZipStream chunked mode.                                   | 05-auxiliary #5          | Low        | S      | L    | 🔄 **Stage 5d (re-investigated)** — `download.py` already streams via `zipstream-ng`'s sized `ZipStream` composed with Django `FileResponse`.                                                                                                       |

### Tier 5 — high-risk / needs investigation before scheduling

| #   | Item                                                                                                                                                                                                             | Sub-plan          | Status                                                                                                                                                                                          |
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| R1  | **Conditional join demotion in `force_inner_joins`.** FTS5 semantics may depend on INNER JOIN; confirm with real FTS5 tests before touching.                                                                     | 03-filters #5     | ✅ **Stage 5.9** — investigation closed: contract is load-bearing, locked in by `tests/test_search_fts.py::FTSForceInnerJoinsTestCase` (4 tests: positive, negative, end-to-end, failure-mode). |
| R2  | **Replace distinct Sum bookmark `Case`.** The comment at `annotate/bookmark.py:57` ("distinct breaks this sum and only returns one") flags correctness risk; any refactor must reproduce the original semantics. | 02-annotations #6 | ❌ **Skipped.** Current code is correct; the Case-Sum is the documented workaround for the distinct interaction. Explicitly dropped from backlog per `05-replan.md` §3.                         |
| R3  | **Serializer audit for N+1.** `ComicSerializer` parent class may have `SerializerMethodField`s that trigger lazy FK access; out of scope for this plan but must happen before pushing metadata optimizations.    | 04-metadata #9    | ⏭️ **Deferred** to a follow-up task — handoff brief at `tasks/browser-views-perf/stage5e-handoff-serializer-audit.md` (scoped, repro queries + benchmark plan, ranked candidate list).          |

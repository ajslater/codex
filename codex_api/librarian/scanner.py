...
from codex_api.librarian.importer import COMIC_MATCHER
...
from codex_api.models import SCHEMA_VERSION
...
def scan_for_new(library):
    ...
    num_skipped = 0
    num_passed = 0
    for root, dirnames, filenames in os.walk(library.path):
        walk_root = Path(root)
...
def scan_existing(library, cls, force=False):
    ...
    num_updated = 0
    num_passed = 0
    objs = cls.objects.filter(library=library)
    for obj in objs:
        path = Path(obj.path)
        task = None
...
def scan_cron():
    ...
    librarys = Library.objects.filter(enable_scan_cron=True)

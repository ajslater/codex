...
from codex_api.serializers.read import ComicReaderInfoSerializer
from codex_api.serializers.read import ComicReaderSettingsSerializer
...
NOTFOUND_IMG_FN = "static/404.png"
...
class ComicOpenedView(APIView, SessionMixin, UserBookmarkMixin):
    ...
    def get_prev_next_comics(self):
        ...
        """
        pk = self.kwargs.get("pk")
        comics = Comic.objects.filter(series__comic=pk).order_by(
            "series__name", "volume__name", "issue"
        )
...
class ComicReaderView(ComicOpenedView):
    """Get context for displaying comic reader toolbar."""
...
class ComicPageView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            LOG.exception(exc)
            page_image = open(NOTFOUND_IMG_FN, "rb")
            return HttpResponse(page_image, content_type="image/png")

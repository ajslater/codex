...
- Upgrading
    - Docker Hub users: `docker.io/ajslater/codex` is published again, but it is
      ...
      changes. Admins see a warning in the web UI, and the logs warn at startup
      and once a day, until the image is changed. Coming from 1.9.x, back up
...
- Fixes
    - Tagging a comic with renaming enabled no longer fails the import and
      ...
      Update Tags to pick up their metadata.
    - Comics renamed or moved by another program mid-scan no longer leave a
      permanent failed import.

...
def _to_vuetify_item(value, title, is_default):
    vuetfy_dict = {
        "value": value,
        "title": title,
    }
    if is_default:
        vuetfy_dict["default"] = True
    return vuetfy_dict
...
def _to_vuetify_dict(fn, data):
    ...
    for key, obj in data.items():
        if isinstance(obj, Mapping):
            ...
            vuetify_list = []
            for value, title in obj.items():
                vuetify_dict = _to_vuetify_item(value, title, default == value)
                vuetify_list.append(vuetify_dict)
            vuetify_value = vuetify_list
        else:
            vuetify_value = obj
...
def _mapping_to_dict(data):
    """Convert nested Mapping objects to dicts."""
    if isinstance(data, Mapping):
        return {key: _mapping_to_dict(value) for key, value in data.items()}
    if isinstance(data, list | tuple | frozenset | set):
        return [_mapping_to_dict(item) for item in data]
    return data


def _dump(parent_path, fn, data, vuetify=True):
    vuetify_data = _to_vuetify_dict(fn, data) if vuetify else _mapping_to_dict(data)

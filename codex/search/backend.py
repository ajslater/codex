...
class CodexSearchBackend(CodexBackendSearchMixin, WhooshSearchBackend, WorkerBaseMixin):
    ...
    _DECIMAL_FIELD_TYPES = frozenset({"decimal", "float"})
    _DATETIME_FIELD_TYPES = frozenset({"date", "datetime"})
    ...
    def build_schema(self, fields):
        ...
        for field_class in fields.values():
            ...
            elif field_class.field_type in self._DECIMAL_FIELD_TYPES:
                schema_fields[index_fieldname] = NUMERIC(
                    numtype=float,  # type: ignore
                    field_boost=field_class.boost,
                    stored=field_class.stored,
                )
            elif field_class.field_type in self._DATETIME_FIELD_TYPES:

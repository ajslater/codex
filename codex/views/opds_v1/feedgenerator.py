...
class OPDSFeedGenerator(Atom1Feed):
    ...
    def root_attributes(self):
        ...
        if self.feed.get("is_aquisition_feed"):
    ...
    def add_root_elements(self, handler: SimplerXMLGenerator):
        ...
        if self.feed.get("use_facets"):
            self._add_facets(handler)
    ...
    def add_item_elements(self, handler: SimplerXMLGenerator, item):
        ...
        if group == "c":
            ...
            extra_qps = card.get("query_params")
            group_link_kwargs = {
                "href": self._add_query_params_to_href(group_link, True, extra_qps)
            }

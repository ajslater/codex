"""
Register the pending-delete reaper's status code.

``LibrarianStatus.status_type`` carries the enum as ``choices``, and
``init_librarian_statuses()`` get-or-creates a row per code at boot while
``_delete_orphans`` removes codes no longer in the enum -- so a new
janitor job needs the column's choices updated or its status row can
never exist.

``max_length`` is derived from the choice *keys*, which are all three
characters, so the column width does not move.
"""
...
class Migration(migrations.Migration):
    """Add JRP to the status type choices."""

    dependencies = [
        ("codex", "0055_missing_since"),
    ]
